...
-- Whether pet-cooldown detection (which relies on COMBAT_LOG_EVENT_UNFILTERED)
-- is currently wanted. Retail refuses both RegisterEvent AND UnregisterEvent
-- for that specific event once it's tainted, so this flag gates the handler
-- instead of the event registration itself ever being toggled off again -
-- see UpdateRegisteredEvents() and eventFrame:COMBAT_LOG_EVENT_UNFILTERED().
local petCooldownEventsEnabled = false
...
function eventFrame:CombatLogEvent(timestamp, event, hideCaster, sourceGUID, sourceName, sourceFlags, sourceRaidFlags, recipientGUID, recipientName, recipientFlags, recipientRaidFlags, skillID)
	if not petCooldownEventsEnabled then
		return
	end
...
local function Disable()
	...
	-- Unregister everything except COMBAT_LOG_EVENT_UNFILTERED (see
	-- UpdateRegisteredEvents() above for why); petCooldownEventsEnabled
	-- already gates its handler regardless.
	petCooldownEventsEnabled = false
