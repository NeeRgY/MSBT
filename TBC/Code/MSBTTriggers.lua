...
-- Whether trigger parsing is currently enabled. COMBAT_LOG_EVENT_UNFILTERED
-- stays registered for the rest of the session once granted (see Disable()
-- below), so this flag - not the event registration - gates combat log
-- trigger processing while disabled.
local isTriggersEnabled = false
...
local function HandleHealthAndPowerTriggers(unit, event, currentAmount, maxAmount, powerType)
	...
	-- Blizzard's "Secret Values" system can return currentAmount/maxAmount as
	-- opaque values on a tainted execution path, which error on arithmetic.
	-- issecretvalue() only exists on clients that have this system at all.
	if issecretvalue and (issecretvalue(currentAmount) or issecretvalue(maxAmount)) then
		return
	end
...
local function OnEvent(this, event, arg1, arg2, ...)
	...
	elseif event == "COMBAT_LOG_EVENT_UNFILTERED" then
		-- This event can't be unregistered again once granted (see
		-- Disable()), so explicitly ignore it while disabled instead.
		if not isTriggersEnabled then return end
...
local function Disable()
	...
	-- Unregister all of the events from the event frame, except
	-- COMBAT_LOG_EVENT_UNFILTERED (see RegisterListenEvents() above for why).
	for event in pairs(listenEvents) do
		if event ~= "COMBAT_LOG_EVENT_UNFILTERED" then
			eventFrame:UnregisterEvent(event)
		end
