...
-- ****************************************************************************
-- Validates the name entered to import a profile into.
-- ****************************************************************************
local function GeneralTab_ValidateImportProfileName(profileName)
	if (not profileName or profileName == "") then
		return L.MSG_INVALID_PROFILE_NAME
	end
...
-- ****************************************************************************
-- Imports the entered profile data into a new profile with the entered name.
-- ****************************************************************************
local function GeneralTab_ImportProfile(profileName, profileData)
	local controls = tabFrames.general.controls

	MSBTProfiles.ImportProfile(profileName, profileData)
...
local function GeneralTab_Create()
	...
	-------------------------------------------------------------------------
	-- General: enable checkboxes.
	-------------------------------------------------------------------------
	MSBTOptMain.Stack(tabFrame, Theme:Header(tabFrame, "General"))

	-- Enable + Blizzard damage checkboxes share one row, side by side.
	local enableRow = CreateFrame("Frame", nil, tabFrame)
	enableRow:SetHeight(Theme.ROW_H)

	local checkbox = MSBTControls.CreateCheckbox(enableRow)
	local objLocale = L.CHECKBOXES["enableMSBT"]
	checkbox:Configure(16, objLocale.label, objLocale.tooltip)
	checkbox:SetPoint("LEFT", enableRow, "LEFT", 0, 0)
	...
	checkbox = MSBTControls.CreateCheckbox(enableRow)
	objLocale = L.CHECKBOXES["enableBlizzardDamage"]
	checkbox:Configure(16, objLocale.label, objLocale.tooltip)
	checkbox:SetPoint("LEFT", controls.enableCheckbox, "RIGHT", 20, 0)
	checkbox:SetClickHandler(
		function (this, isChecked)
	...
	MSBTOptMain.Stack(tabFrame, enableRow)
	...
	if not IsClassic then
		...
		checkbox:Configure(16, objLocale.label, objLocale.tooltip)
		checkbox:SetHeight(Theme.ROW_H)
		checkbox:SetClickHandler(
			function (this, isChecked)
				if InCombatLockdown() then
					return
		...
		MSBTOptMain.Stack(tabFrame, checkbox)
	end

	-------------------------------------------------------------------------
	-- Profile: dropdown, copy/reset/delete buttons, animation speed.
	-------------------------------------------------------------------------
	MSBTOptMain.Stack(tabFrame, Theme:Header(tabFrame, "Profile"))
	...
	MSBTOptMain.Stack(tabFrame, dropdown)

	-- Copy/reset/delete profile buttons share one row.
	local profileButtonsRow = CreateFrame("Frame", nil, tabFrame)
	profileButtonsRow:SetHeight(24)

	local button = MSBTControls.CreateOptionButton(profileButtonsRow)
	objLocale = L.BUTTONS["copyProfile"]
	button:Configure(24, objLocale.label, objLocale.tooltip)
	button:SetPoint("LEFT", profileButtonsRow, "LEFT", 0, 0)
	button:SetClickHandler(
		function (this)
	...
	button = MSBTControls.CreateOptionButton(profileButtonsRow)
	...
	MSBTOptMain.Stack(tabFrame, profileButtonsRow)

	-- Export/import profile buttons share their own row.
	local profileShareRow = CreateFrame("Frame", nil, tabFrame)
	profileShareRow:SetHeight(24)

	button = MSBTControls.CreateOptionButton(profileShareRow)
	objLocale = L.BUTTONS["exportProfile"]
	button:Configure(24, objLocale.label, objLocale.tooltip)
	button:SetPoint("LEFT", profileShareRow, "LEFT", 0, 0)
	button:SetClickHandler(
		function (this)
			EraseTable(configTable)
			configTable.exportString = MikSBT.ExportString(MSBTProfiles.currentProfile)
			configTable.parentFrame = tabFrame
			configTable.anchorFrame = this
			configTable.hideHandler = GeneralTab_EnableControls
			DisableControls(controls)
			MSBTPopups.ShowProfileExport(configTable)
		end
	)
	controls.exportProfileButton = button

	button = MSBTControls.CreateOptionButton(profileShareRow)
	objLocale = L.BUTTONS["importProfile"]
	button:Configure(24, objLocale.label, objLocale.tooltip)
	...
	button:SetClickHandler(
		function (this)
			...
			configTable.parentFrame = tabFrame
			configTable.anchorFrame = this
			configTable.validateNameHandler = GeneralTab_ValidateImportProfileName
			configTable.saveHandler = GeneralTab_ImportProfile
			configTable.hideHandler = GeneralTab_EnableControls
			DisableControls(controls)
			MSBTPopups.ShowProfileImport(configTable)
		end
	)
	controls.importProfileButton = button

	MSBTOptMain.Stack(tabFrame, profileShareRow)
	...
	MSBTOptMain.Stack(tabFrame, slider)

	-------------------------------------------------------------------------
	-- Display: text shadowing / sounds / sticky crits checkboxes.
	-------------------------------------------------------------------------
	MSBTOptMain.Stack(tabFrame, Theme:Header(tabFrame, "Display"))

	local displayRow = CreateFrame("Frame", nil, tabFrame)
	displayRow:SetHeight(Theme.ROW_H)

	checkbox = MSBTControls.CreateCheckbox(displayRow)
	objLocale = L.CHECKBOXES["enableSounds"]
	checkbox:Configure(16, objLocale.label, objLocale.tooltip)
	checkbox:SetPoint("LEFT", displayRow, "LEFT", 0, 0)
	checkbox:SetClickHandler(
		function (this, isChecked)
	...
	checkbox:Configure(16, objLocale.label, objLocale.tooltip)
	checkbox:SetPoint("LEFT", controls.enableSoundsCheckbox, "RIGHT", 20, 0)
	checkbox:SetClickHandler(
		function (this, isChecked)
	...
	MSBTOptMain.Stack(tabFrame, displayRow)
	...
	-------------------------------------------------------------------------
	-- Customize: class/damage colors, partial effects, master font.
	-------------------------------------------------------------------------
	MSBTOptMain.Stack(tabFrame, Theme:Header(tabFrame, "Customize"))
	...
	MSBTOptMain.Stack(tabFrame, button)
...
local function ScrollAreasTab_CreateLine(this)
	...
	checkbox:Configure(16, nil, objLocale.tooltip)
...
local function SpamTab_Create()
	...
	checkbox:Configure(16, objLocale.label, objLocale.tooltip)
	checkbox:SetPoint("TOPLEFT", tabFrame, "TOPLEFT", 5, -148)
	checkbox:SetClickHandler(
		function (this, isChecked)
	...
	checkbox:Configure(16, objLocale.label, objLocale.tooltip)
	checkbox:SetPoint("TOPLEFT", controls.hideSkillsCheckbox, "BOTTOMLEFT", 0, -6)
	checkbox:SetClickHandler(
		function (this, isChecked)
	...
	checkbox:Configure(16, objLocale.label, objLocale.tooltip)
	checkbox:SetPoint("TOPLEFT", controls.hideNamesCheckbox, "BOTTOMLEFT", 0, -6)
	checkbox:SetClickHandler(
		function (this, isChecked)
	...
	checkbox:Configure(16, objLocale.label, objLocale.tooltip)
	checkbox:SetPoint("TOPLEFT", controls.hideFullOverhealsCheckbox, "BOTTOMLEFT", 0, -6)
	checkbox:SetClickHandler(
		function (this, isChecked)
	...
	checkbox:Configure(16, objLocale.label, objLocale.tooltip)
	checkbox:SetPoint("TOPLEFT", controls.hideFullHoTOverhealsCheckbox, "BOTTOMLEFT", 0, -6)
	checkbox:SetClickHandler(
		function (this, isChecked)
	...
	checkbox:Configure(16, objLocale.label, objLocale.tooltip)
	checkbox:SetPoint("TOPLEFT", controls.allPowerCheckbox, "BOTTOMLEFT", 0, -6)
	checkbox:SetClickHandler(
		function (this, isChecked)
	...
	checkbox:Configure(16, objLocale.label, objLocale.tooltip)
	checkbox:SetPoint("TOPLEFT", controls.abbreviateCheckbox, "BOTTOMLEFT", 0, -6)
	checkbox:SetClickHandler(
		function (this, isChecked)
	...
	checkbox:Configure(16, objLocale.label, objLocale.tooltip)
	checkbox:SetPoint("TOPLEFT", controls.mergeSwingsCheckbox, "BOTTOMLEFT", 0, -6)
	checkbox:SetClickHandler(
		function (this, isChecked)
	...
	checkbox:Configure(16, objLocale.label, objLocale.tooltip)
	checkbox:SetPoint("TOPLEFT", controls.shortenNumbersCheckbox, "BOTTOMLEFT", 0, -6)
	checkbox:SetClickHandler(
		function (this, isChecked)
...
local function CooldownsTab_Create()
	...
	-- Cooldown Always button.
	button = MSBTControls.CreateOptionButton(tabFrame)
	objLocale = L.BUTTONS["ignoreCooldownThreshold"]
	button:Configure(20, objLocale.label, objLocale.tooltip)
	button:SetPoint("TOPLEFT", controls.cooldownExclusions, "BOTTOMLEFT", 0, -20)
	button:SetClickHandler(
		function (this)
			local listName = "ignoreCooldownThreshold"
			PopulateList(listName)
			EraseTable(configTable)
			configTable.title = this:GetText()
			configTable.skills = listTable
	...
	controls.ignoreCooldownThreshold = button
...
local function LootAlertsTab_Create()
	...
	for quality = LE_ITEM_QUALITY_POOR or Enum.ItemQuality.Poor, LE_ITEM_QUALITY_EPIC or Enum.ItemQuality.Epic do
		...
		checkbox:Configure(16, label, L.MSG_DISPLAY_QUALITY)
...
-- The addon's own UI languages, independent of the game client's locale -
-- 1:1 with the Localization/localization.*.lua files, which each accept a
-- MSBTProfiles_SavedVars.uiLanguage override in place of GetLocale(). Native
-- language names, like any language picker, aren't translated per-locale.
-- Russian/Chinese/Korean are spelled out in English (not their own native
-- script) deliberately: the client only loads glyph data for its own
-- locale's font, so Cyrillic/CJK text shows up as blank tofu boxes on a
-- client running any other locale (e.g. this list on an enUS client) -
-- English is the one script every client can always render.
local LANGUAGES = {
	{"auto", "Automatic"},
	{"enUS", "English"},
	{"deDE", "Deutsch"},
...
-- ****************************************************************************
-- Enables the controls on the settings tab.
-- ****************************************************************************
local function SettingsTab_EnableControls()
	for name, frame in pairs(tabFrames.settings.controls) do
		if (frame.Enable) then frame:Enable() end
	end
end
...
-- ****************************************************************************
-- Prompts to reload the UI, e.g. after a setting that only takes effect on
-- the next load (such as the addon's language) was changed.
-- ****************************************************************************
local function SettingsTab_PromptReload(anchorFrame)
	local tabFrame = tabFrames.settings
	local controls = tabFrame.controls
	EraseTable(configTable)
	configTable.parentFrame = tabFrame
	configTable.anchorFrame = anchorFrame
	configTable.acknowledgeHandler = ReloadUI
	configTable.hideHandler = SettingsTab_EnableControls
	DisableControls(controls)
	MSBTPopups.ShowAcknowledge(configTable)
end
...
-- ****************************************************************************
-- Resets every MSBT setting and profile back to its defaults (everything
-- except imported custom fonts/sounds) and reloads the UI to rebuild them.
-- ****************************************************************************
local function SettingsTab_ResetEverything()
	-- Nil out the globals themselves rather than just clearing their
	-- contents: MSBTProfiles.InitSavedVariables() (see MSBTProfiles.lua)
	-- only rebuilds the default profile from scratch when the SavedVariables
	-- global itself is missing, not merely empty.
	MSBTProfiles_SavedVars = nil
...
-- ****************************************************************************
-- Creates the settings tab frame contents.
-- ****************************************************************************
local function SettingsTab_Create()
	local tabFrame = tabFrames.settings
	tabFrame.controls = {}
	local controls = tabFrame.controls
	tabFrame._y = 0

	MSBTOptMain.Stack(tabFrame, Theme:Header(tabFrame, L.TABS.settings.label))

	-- Accent color: opens the custom KeyHerald-style color picker and
	-- re-tints every already-created widget immediately via
	-- Theme:OnHighlightColorChanged(). A small reset button next to it
	-- restores KeyHerald's default violet.
	local colorRow = CreateFrame("Frame", nil, tabFrame)
	colorRow:SetHeight(Theme.ROW_H)

	local objLocale = L.CHECKBOXES["uiAccentColor"]
	local swatch = MSBTControls.CreateColorswatch(colorRow)
	swatch:SetPoint("LEFT", colorRow, "LEFT", 0, 0)
	do
		local r, g, b = Theme:GetHighlightColor()
		swatch:SetColor(r, g, b)
	end
	swatch:SetTooltip(objLocale.tooltip)
	swatch:SetColorChangedHandler(function(this)
		Theme:SetHighlightColor(this.r, this.g, this.b)
	end)
	controls.accentSwatch = swatch

	local label = Theme:FontString(colorRow)
	label:SetPoint("LEFT", swatch, "RIGHT", 8, 0)
	label:SetText(objLocale.label)
	label:SetTextColor(unpack(Theme.text))

	local resetColorButton = MSBTControls.CreateOptionButton(colorRow)
	objLocale = L.BUTTONS["resetAccentColor"]
	resetColorButton:Configure(20, objLocale.label, objLocale.tooltip)
	resetColorButton:SetPoint("LEFT", label, "RIGHT", 12, 0)
	resetColorButton:SetClickHandler(function(this)
		Theme:SetHighlightColor(unpack(Theme.DEFAULT_HIGHLIGHT))
		local r, g, b = Theme:GetHighlightColor()
		controls.accentSwatch:SetColor(r, g, b)
	end)

	MSBTOptMain.Stack(tabFrame, colorRow)

	-- Addon language.
	local languageDropdown = MSBTControls.CreateDropdown(tabFrame)
	objLocale = L.DROPDOWNS["language"]
	languageDropdown:Configure(180, objLocale.label, objLocale.tooltip)
	for _, entry in ipairs(LANGUAGES) do
		languageDropdown:AddItem(entry[2], entry[1])
	end
	languageDropdown:SetSelectedID((MSBTProfiles_SavedVars and MSBTProfiles_SavedVars.uiLanguage) or "auto")
	languageDropdown:SetChangeHandler(function(this, id)
		MSBTProfiles_SavedVars.uiLanguage = id
		SettingsTab_PromptReload(this)
	end)
	controls.languageDropdown = languageDropdown
	MSBTOptMain.Stack(tabFrame, languageDropdown)

	-- Minimap button.
	local minimapCheckbox = MSBTControls.CreateCheckbox(tabFrame)
	objLocale = L.CHECKBOXES["showMinimapButton"]
	minimapCheckbox:Configure(16, objLocale.label, objLocale.tooltip)
	minimapCheckbox:SetHeight(Theme.ROW_H)
	minimapCheckbox:SetChecked(MikSBT.MinimapButton.IsShown())
	minimapCheckbox:SetClickHandler(function(this, isChecked)
		MikSBT.MinimapButton.SetShown(isChecked)
	end)
	controls.minimapCheckbox = minimapCheckbox
	MSBTOptMain.Stack(tabFrame, minimapCheckbox)

	-- Window scale.
	local scaleSlider = MSBTControls.CreateSlider(tabFrame)
	objLocale = L.SLIDERS["windowScale"]
	scaleSlider:Configure(180, objLocale.label, objLocale.tooltip)
	scaleSlider:SetMinMaxValues(70, 150)
	scaleSlider:SetValueStep(5)
	scaleSlider:SetValue(math.floor(MSBTOptMain.GetWindowScale() * 100 + 0.5))
	scaleSlider:SetValueChangedHandler(function(this, value)
		MSBTOptMain.SetWindowScale(value / 100)
	end)
	controls.scaleSlider = scaleSlider
	MSBTOptMain.Stack(tabFrame, scaleSlider)

	-- Window transparency. Floor of 40% so the window can't be slid down to
	-- fully invisible while still blocking clicks underneath it.
	local alphaSlider = MSBTControls.CreateSlider(tabFrame)
	objLocale = L.SLIDERS["windowAlpha"]
	alphaSlider:Configure(180, objLocale.label, objLocale.tooltip)
	alphaSlider:SetMinMaxValues(40, 100)
	alphaSlider:SetValueStep(5)
	alphaSlider:SetValue(math.floor(MSBTOptMain.GetWindowAlpha() * 100 + 0.5))
	alphaSlider:SetValueChangedHandler(function(this, value)
		MSBTOptMain.SetWindowAlpha(value / 100)
	end)
	controls.alphaSlider = alphaSlider
	MSBTOptMain.Stack(tabFrame, alphaSlider)

	-- Reset everything.
	local resetRow = CreateFrame("Frame", nil, tabFrame)
	resetRow:SetHeight(24)

	local resetButton = MSBTControls.CreateOptionButton(resetRow)
	objLocale = L.BUTTONS["resetEverything"]
	resetButton:Configure(24, objLocale.label, objLocale.tooltip)
	resetButton:SetPoint("LEFT", resetRow, "LEFT", 0, 0)
	resetButton:SetClickHandler(function(this)
		EraseTable(configTable)
		configTable.parentFrame = tabFrame
		configTable.anchorFrame = this
		configTable.acknowledgeHandler = SettingsTab_ResetEverything
		configTable.hideHandler = SettingsTab_EnableControls
		DisableControls(controls)
		MSBTPopups.ShowAcknowledge(configTable)
	end)

	MSBTOptMain.Stack(tabFrame, resetRow, 8)
...
-- Called when the tab frame is shown.
-- ****************************************************************************
local function SettingsTab_OnShow()
	if (not tabFrames.settings.created) then SettingsTab_Create() end

	local controls = tabFrames.settings.controls
	local r, g, b = Theme:GetHighlightColor()
	controls.accentSwatch:SetColor(r, g, b)
	controls.minimapCheckbox:SetChecked(MikSBT.MinimapButton.IsShown())
	controls.scaleSlider:SetValue(math.floor(MSBTOptMain.GetWindowScale() * 100 + 0.5))
	controls.alphaSlider:SetValue(math.floor(MSBTOptMain.GetWindowAlpha() * 100 + 0.5))
end
...
-- Search index.
-------------------------------------------------------------------------------
-- Every checkbox/slider/dropdown/button/editbox label used by each tab,
-- referenced by locale group + key rather than duplicating the strings, so
-- the search box (see MSBTOptionsMain.lua's search field) matches on
-- whatever language is actually active instead of a hardcoded English copy.
-- Built from the actual *_Create() functions below, not by hand.
-------------------------------------------------------------------------------

local TAB_SEARCH_KEYS = {
	customMedia = {
		{"BUTTONS", "addCustomFont"}, {"BUTTONS", "addCustomSound"},
		{"EDITBOXES", "customFontName"}, {"EDITBOXES", "customFontPath"},
		{"EDITBOXES", "customSoundName"}, {"EDITBOXES", "customSoundPath"},
	},
	general = {
		{"BUTTONS", "classColors"}, {"BUTTONS", "copyProfile"}, {"BUTTONS", "damageColors"},
		{"BUTTONS", "deleteProfile"}, {"BUTTONS", "deleteScrollArea"}, {"BUTTONS", "editScrollAreaName"},
		{"BUTTONS", "exportProfile"}, {"BUTTONS", "importProfile"}, {"BUTTONS", "masterFont"},
		{"BUTTONS", "partialEffects"}, {"BUTTONS", "resetProfile"}, {"BUTTONS", "scrollAreaFontSettings"},
		{"CHECKBOXES", "enableBlizzardDamage"}, {"CHECKBOXES", "enableBlizzardHealing"},
		{"CHECKBOXES", "enableMSBT"}, {"CHECKBOXES", "enableScrollArea"}, {"CHECKBOXES", "enableSounds"},
		{"CHECKBOXES", "stickyCrits"}, {"CHECKBOXES", "textShadowing"},
		{"DROPDOWNS", "profile"}, {"EDITBOXES", "copyProfile"}, {"EDITBOXES", "scrollAreaName"},
		{"SLIDERS", "animationSpeed"},
	},
	scrollAreas = {
		{"BUTTONS", "addScrollArea"}, {"BUTTONS", "configScrollAreas"},
		{"BUTTONS", "eventFontSettings"}, {"BUTTONS", "eventSettings"}, {"EDITBOXES", "scrollAreaName"},
	},
	events = {
		{"BUTTONS", "deleteTrigger"}, {"BUTTONS", "eventFontSettings"}, {"BUTTONS", "eventSettings"},
		{"BUTTONS", "moveAll"}, {"BUTTONS", "toggleAll"}, {"BUTTONS", "triggerSettings"},
		{"CHECKBOXES", "enableTrigger"}, {"DROPDOWNS", "eventCategory"},
	},
	triggers = {
		{"BUTTONS", "addTrigger"}, {"EDITBOXES", "eventMessage"},
	},
	spamControl = {
		{"BUTTONS", "mergeExclusions"}, {"BUTTONS", "skillSubstitutions"}, {"BUTTONS", "skillSuppressions"},
		{"BUTTONS", "throttleList"}, {"CHECKBOXES", "abbreviateSkills"}, {"CHECKBOXES", "allPowerGains"},
		{"CHECKBOXES", "groupNumbers"}, {"CHECKBOXES", "hideFullHoTOverheals"}, {"CHECKBOXES", "hideFullOverheals"},
		{"CHECKBOXES", "hideMergeTrailer"}, {"CHECKBOXES", "hideNames"}, {"CHECKBOXES", "hideSkills"},
		{"CHECKBOXES", "mergeSwings"}, {"CHECKBOXES", "shortenNumbers"},
		{"SLIDERS", "damageThreshold"}, {"SLIDERS", "dotThrottleTime"}, {"SLIDERS", "healThreshold"},
		{"SLIDERS", "hotThrottleTime"}, {"SLIDERS", "powerThreshold"}, {"SLIDERS", "powerThrottleTime"},
	},
	cooldowns = {
		{"BUTTONS", "cooldownExclusions"}, {"BUTTONS", "eventFontSettings"}, {"BUTTONS", "eventSettings"},
		{"BUTTONS", "ignoreCooldownThreshold"}, {"CHECKBOXES", "enableItemCooldowns"},
		{"CHECKBOXES", "enablePetCooldowns"}, {"CHECKBOXES", "enablePlayerCooldowns"},
		{"SLIDERS", "cooldownThreshold"},
	},
	lootAlerts = {
		{"BUTTONS", "eventFontSettings"}, {"BUTTONS", "eventSettings"}, {"BUTTONS", "itemExclusions"},
		{"BUTTONS", "itemsAllowed"}, {"CHECKBOXES", "alwaysShowQuestItems"}, {"CHECKBOXES", "currencyGains"},
		{"CHECKBOXES", "lootedItems"}, {"CHECKBOXES", "moneyGains"},
	},
	skillIcons = {
		{"CHECKBOXES", "enableIcons"}, {"CHECKBOXES", "exclusiveSkills"},
	},
	settings = {
		{"CHECKBOXES", "uiAccentColor"}, {"BUTTONS", "resetAccentColor"},
		{"DROPDOWNS", "language"}, {"CHECKBOXES", "showMinimapButton"},
		{"SLIDERS", "windowScale"}, {"SLIDERS", "windowAlpha"},
		{"BUTTONS", "resetEverything"},
	},
}

-- ****************************************************************************
-- Builds the lowercased search text for one tab: its own name plus every
-- label referenced in TAB_SEARCH_KEYS for it.
-- ****************************************************************************
local function BuildTabSearchText(tabKey)
	local locGroups = {
		CHECKBOXES = L.CHECKBOXES, SLIDERS = L.SLIDERS, DROPDOWNS = L.DROPDOWNS,
		BUTTONS = L.BUTTONS, EDITBOXES = L.EDITBOXES,
	}
	local tabLocale = L.TABS[tabKey]
	local parts = { tabLocale and tabLocale.label or "" }
	for _, entry in ipairs(TAB_SEARCH_KEYS[tabKey] or {}) do
		local obj = locGroups[entry[1]][entry[2]]
		if (obj and obj.label) then
			parts[#parts + 1] = obj.label
		end
	end
