...
-- Flag indicating a deferred Blizzard-combat-text disable is waiting for combat to end.
local pendingFCTDisable
...
-- ****************************************************************************
-- Every CVar that feeds Blizzard's own floating combat text. Some of these
-- don't exist on every client (e.g. the "_v2" nameplate-driven generation is
-- retail-only, some pet/absorb CVars aren't in Classic), so each one is
-- checked with GetCVarDefault() before being touched instead of branching on
-- IsClassic/IsCataClassic.
local blizzardFCTCVars = {
...
-- Several floatingCombatTextXXX CVars have a matching GLOBAL VARIABLE that
-- the "on me" (self) combat-text engine actually reads - e.g. Miss/Dodge/
-- Parry's real switch is the global COMBAT_TEXT_SHOW_DODGE_PARRY_MISS, not
-- just SHOW_COMBAT_TEXT (which is only the master "on me" toggle).
local blizzardFCTGlobalForCVar = {
...
local function BlizzardFCTCVarExists(cvar)
	return GetCVarDefault(cvar) ~= nil
end

local function SetBlizzardFCTCVar(cvar, on)
	-- Prefer the "_v2" variant when it exists (drives the newer
	-- nameplate-anchored damage/heal numbers on retail); fall back to the
	-- plain CVar, and skip entirely if neither exists on this client.
	local realCVar = cvar
	if BlizzardFCTCVarExists(cvar .. "_v2") then
		realCVar = cvar .. "_v2"
	elseif not BlizzardFCTCVarExists(cvar) then
		return
	end

	SetCVar(realCVar, on and "1" or "0")
	local globalVar = blizzardFCTGlobalForCVar[cvar]
	if globalVar then _G[globalVar] = on and "1" or "0" end
end

local function ApplyDisableBlizzardCombatText()
	-- SetCVar-ing enableFloatingCombatText straight to the desired value
	-- doesn't reliably get picked up live; flipping it through "1" first
	-- forces the engine to re-read it.
	SetBlizzardFCTCVar("enableFloatingCombatText", true)
	SetBlizzardFCTCVar("enableFloatingCombatText", false)
	for _, cvar in ipairs(blizzardFCTCVars) do
		if cvar ~= "enableFloatingCombatText" then
			SetBlizzardFCTCVar(cvar, false)
		end
	end

	-- SetCVar alone doesn't make Blizzard's already-running CombatText frame
	-- re-read these settings; the default UI's own options panel explicitly
	-- pokes it via CombatText_UpdateDisplayedMessages() after changing CVars.
...
local function DisableBlizzardCombatText()
	...
	-- Defer and retry once combat ends via PLAYER_REGEN_ENABLED.
	if (InCombatLockdown()) then
		pendingFCTDisable = true
		return
	end
	ApplyDisableBlizzardCombatText()
end
...
-- ****************************************************************************
-- Creates a new profile named destProfileName from a profile settings table
-- (as produced by MikSBT.ImportString), the way an imported profile string
-- gets turned into an actual profile.
-- ****************************************************************************
local function ImportProfile(destProfileName, profileSettings)
	if (not destProfileName or destProfileName == "") then return end
	if (type(profileSettings) ~= "table") then return end
	if (savedVariables.profiles[destProfileName]) then return end

	local newProfile = CopyTable(profileSettings)
	newProfile.creationVersion = MikSBT.VERSION .. "." .. MikSBT.SVN_REVISION
	savedVariables.profiles[destProfileName] = newProfile
end
...
local function InitSavedVariables()
	...
	-- Create a new table to hold the saved variables if there wasn't one.
	if (not savedVariables) then
		savedVariables = {}
		_G[SAVED_VARS_NAME] = savedVariables
	end

	-- Check for a missing or empty profiles table - either a genuinely fresh
	-- install, or one left behind by a "reset everything"-style wipe that
	-- clears the table's contents without nil-ing out the SavedVariables
	-- global itself. Only the profiles table is rebuilt here, so any other
	-- top-level data already in savedVariables (e.g. the options window's
	-- saved theme) survives either way.
	if (not savedVariables.profiles or not next(savedVariables.profiles)) then
...
local function OnEvent(this, event, arg1)
	...
	if (event == "ADDON_LOADED") then
		...
		if (MikSBT.localePacksCore) then
			...
			if (effective ~= "enUS" and MikSBT.localePacksCore[effective]) then
				MikSBT.localePacksCore[effective]()
			end
		end
	...
	elseif (event == "VARIABLES_LOADED") then
		...
		-- Disable Blizzard's combat text every login (not just on first
		-- install): SavedVariables carried over from an earlier MSBT
		-- install, or Blizzard itself resetting the CVars on a patch, both
		-- left enableFloatingCombatText back on with isFirstLoad never
		-- true again, so Blizzard's own numbers kept showing alongside (or
		-- instead of) MSBT's.
	...
	-- Combat ended: retry any Blizzard-combat-text disable that was deferred
	-- because SetCVar silently refuses to apply while in combat.
	elseif (event == "PLAYER_REGEN_ENABLED") then
		if (pendingFCTDisable) then
			pendingFCTDisable = false
			ApplyDisableBlizzardCombatText()
		end
	end
end
