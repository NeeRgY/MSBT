...
local IsRetail = WOW_PROJECT_ID == WOW_PROJECT_MAINLINE and not MikSBT.Client.isForever
...
-- Whether the parser is currently enabled. COMBAT_LOG_EVENT_UNFILTERED stays
-- registered for the rest of the session once granted (see Disable() below),
-- so this flag - not the event registration - is what actually gates combat
-- log processing while disabled.
local isParserEnabled = false
...
local function OnEvent(this, event, arg1, arg2, ...)
	...
	if event == "COMBAT_LOG_EVENT_UNFILTERED" then
		...
		if not isParserEnabled then return end
