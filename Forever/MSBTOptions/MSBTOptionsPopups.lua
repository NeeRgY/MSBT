...
local function CreatePopup()
	-- Flat KeyHerald-style panel (../KeyHerald's GUI/MainFrame.lua buildWindow())
	-- instead of Blizzard's tooltip-bordered parchment dialog - matches the
	-- rest of the options window. Strata "DIALOG" (above the main window's
	-- "HIGH", below the dropdown listbox/color picker's "FULLSCREEN_DIALOG"
	-- so those still draw correctly on top of a popup that opens them)
	-- instead of "HIGH", which put popups at the same strata as the main
	-- window and left their draw order undefined relative to it.
...
-------------------------------------------------------------------------------
-- Profile export/import frame functions.
-------------------------------------------------------------------------------

-- ****************************************************************************
-- Creates a themed, scrollable multi-line editbox for larger blocks of text
...
local function CreateMultilineEditbox(parent)
	...
	container:SetBackdrop({ bgFile = Theme.flatTexture, edgeFile = Theme.flatTexture, edgeSize = 1 })
	container:SetBackdropColor(unpack(Theme.fill))
	...
	editBox:SetFontObject(Theme.font.small)
	...
	container.SetText = function(self, text) editBox:SetText(text or "") end
	container.GetText = function(self) return editBox:GetText() end
	container.HighlightText = function(self) editBox:HighlightText() end
	container.SetFocus = function(self) editBox:SetFocus() end
	return container
end


-- ****************************************************************************
-- Creates the popup profile export frame.
-- ****************************************************************************
local function CreateProfileExport()
	local frame = CreatePopup()
	frame:SetWidth(440)
	frame:SetHeight(300)
	...
	fontString:SetFontObject(Theme.font.title)
	fontString:SetPoint("TOP", frame, "TOP", 0, -14)
	do
		local r, g, b = Theme:GetHighlightColor()
		fontString:SetTextColor(r, g, b)
	end
	Theme:OnHighlightColorChanged(function(r, g, b) fontString:SetTextColor(r, g, b) end)
	fontString:SetText(L.MSG_EXPORT_PROFILE_TITLE)
	frame.titleFontString = fontString

	-- Hint text.
	local hint = frame:CreateFontString(nil, "OVERLAY")
	hint:SetFontObject(Theme.font.small)
	hint:SetTextColor(unpack(Theme.mutedText))
	hint:SetPoint("TOPLEFT", frame, "TOPLEFT", 20, -40)
	hint:SetJustifyH("LEFT")
	hint:SetText(L.MSG_EXPORT_PROFILE_HINT)

	-- Export text box.
	local textBox = CreateMultilineEditbox(frame)
	textBox:SetPoint("TOPLEFT", frame, "TOPLEFT", 20, -64)
	textBox:SetPoint("TOPRIGHT", frame, "TOPRIGHT", -20, -64)
	textBox:SetHeight(160)
	frame.textBox = textBox

	-- Close button.
	local button = MSBTControls.CreateOptionButton(frame)
	local objLocale = L.BUTTONS["profileExportClose"]
	button:Configure(20, objLocale.label, objLocale.tooltip)
...
-- ****************************************************************************
-- Shows the popup profile export frame using the passed config.
-- ****************************************************************************
local function ShowProfileExport(configTable)
	-- Don't do anything if required parameters weren't passed.
	if (not configTable or not configTable.anchorFrame or not configTable.parentFrame) then return end

	-- Create the frame if it hasn't already been.
	if (not popupFrames.profileExportFrame) then popupFrames.profileExportFrame = CreateProfileExport() end

	-- Set parent.
	local frame = popupFrames.profileExportFrame
	ChangePopupParent(frame, configTable.parentFrame)

	-- Populate.
	frame.textBox:SetText(configTable.exportString)
...
-- ****************************************************************************
-- Called when the profile import name or data changes to allow validation.
-- ****************************************************************************
local function ValidateProfileImport()
	local frame = popupFrames.profileImportFrame

	frame.validateFontString:SetText("")
	frame.importButton:Enable()

	local message
	if (frame.validateNameHandler) then
		message = frame.validateNameHandler(frame.nameEditbox:GetText())
	end
	if (not message) then
		local data = frame.dataTextBox:GetText()
		if (not data or data == "" or not MikSBT.ImportString(data)) then
			message = L.MSG_INVALID_PROFILE_DATA
		end
	end

	if (message) then
		frame.validateFontString:SetText(message)
		frame.importButton:Disable()
	end
end


-- ****************************************************************************
-- Calls the save handler with the entered name and parsed profile data.
-- ****************************************************************************
local function SaveProfileImport()
	local frame = popupFrames.profileImportFrame
	if (frame.saveHandler and frame.importButton:IsEnabled() ~= 0) then
		local data = MikSBT.ImportString(frame.dataTextBox:GetText())
		if (data) then
			local name = frame.nameEditbox:GetText()
			frame:Hide()
			frame.saveHandler(name, data)
		end
	end
end
...
-- ****************************************************************************
-- Creates the popup profile import frame.
-- ****************************************************************************
local function CreateProfileImport()
	local frame = CreatePopup()
	frame:SetWidth(440)
	frame:SetHeight(320)
	...
	Theme:OnHighlightColorChanged(function(r, g, b) fontString:SetTextColor(r, g, b) end)
	fontString:SetText(L.MSG_IMPORT_PROFILE_TITLE)
	frame.titleFontString = fontString

	-- Profile name editbox.
	local objLocale = L.EDITBOXES["importProfileName"]
	local nameEditbox = MSBTControls.CreateEditbox(frame)
	nameEditbox:SetPoint("TOPLEFT", frame, "TOPLEFT", 20, -45)
	nameEditbox:SetPoint("TOPRIGHT", frame, "TOPRIGHT", -20, -45)
	nameEditbox:SetLabel(objLocale.label)
	nameEditbox:SetTooltip(objLocale.tooltip)
	nameEditbox:SetTextChangedHandler(ValidateProfileImport)
	...
	hint:SetFontObject(Theme.font.small)
	hint:SetTextColor(unpack(Theme.mutedText))
	hint:SetPoint("TOPLEFT", nameEditbox, "BOTTOMLEFT", 0, -14)
	hint:SetJustifyH("LEFT")
	hint:SetText(L.MSG_IMPORT_PROFILE_HINT)

	-- Data text box.
	local dataTextBox = CreateMultilineEditbox(frame)
	dataTextBox:SetPoint("TOPLEFT", nameEditbox, "BOTTOMLEFT", 0, -34)
	dataTextBox:SetPoint("TOPRIGHT", nameEditbox, "BOTTOMRIGHT", 0, -34)
	dataTextBox:SetHeight(130)
	dataTextBox.editBox:SetScript("OnTextChanged", ValidateProfileImport)
	frame.dataTextBox = dataTextBox

	-- Import button.
	local button = MSBTControls.CreateOptionButton(frame)
	objLocale = L.BUTTONS["profileImportOkay"]
	button:Configure(20, objLocale.label, objLocale.tooltip)
	button:SetPoint("BOTTOMRIGHT", frame, "BOTTOM", -10, 15)
	button:SetClickHandler(SaveProfileImport)
	frame.importButton = button

	-- Cancel button.
	button = MSBTControls.CreateOptionButton(frame)
	objLocale = L.BUTTONS["profileImportCancel"]
	button:Configure(20, objLocale.label, objLocale.tooltip)
	...
	-- Validation text.
	local fontString2 = frame:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
	fontString2:SetPoint("BOTTOMLEFT", frame, "BOTTOMLEFT", 30, 40)
	fontString2:SetPoint("BOTTOMRIGHT", frame, "BOTTOMRIGHT", -30, 40)
	frame.validateFontString = fontString2
...
-- ****************************************************************************
-- Shows the popup profile import frame using the passed config.
-- ****************************************************************************
local function ShowProfileImport(configTable)
	-- Don't do anything if required parameters weren't passed.
	if (not configTable or not configTable.anchorFrame or not configTable.parentFrame) then return end

	-- Create the frame if it hasn't already been.
	if (not popupFrames.profileImportFrame) then popupFrames.profileImportFrame = CreateProfileImport() end
	...
	-- Populate.
	frame.nameEditbox:SetText(configTable.defaultText)
	frame.dataTextBox:SetText("")
	frame.validateFontString:SetText("")
	frame.importButton:Enable()

	-- Configure the frame.
	frame.validateNameHandler = configTable.validateNameHandler
	frame.saveHandler = configTable.saveHandler
	frame.hideHandler = configTable.hideHandler
	frame:ClearAllPoints()
	frame:SetPoint(configTable.anchorPoint or "TOPLEFT", configTable.anchorFrame, configTable.relativePoint or "BOTTOMLEFT")
...
local function CreateFontPopup()
	...
	-- Preview button: fires a real animated sample through the same
	-- MikSBT.DisplayMessage() pipeline the Scroll Areas popup's own Preview
	-- button uses, with whatever font/size/outline/opacity is currently
	-- selected (not yet saved) - the static previews above only show the
	-- font shape, not how it actually moves/fades/looks in color.
	button = MSBTControls.CreateOptionButton(frame)
	objLocale = L.BUTTONS["fontPreview"]
	button:Configure(20, objLocale.label, objLocale.tooltip)
	button:SetPoint("BOTTOM", frame, "BOTTOM", 0, 50)
	button:SetClickHandler(function(this)
		UpdateFontSettings()
		if (not frame.hideNormal) then
			MikSBT.DisplayMessage(L.MSG_NORMAL_PREVIEW_TEXT, nil, false, 255, 255, 255,
				returnSettings.normalFontSize, returnSettings.normalFontName, returnSettings.normalOutlineIndex)
		end
		if (not frame.hideCrit) then
			MikSBT.DisplayMessage(L.MSG_CRIT, nil, false, 255, 32, 32,
				returnSettings.critFontSize, returnSettings.critFontName, returnSettings.critOutlineIndex)
		end
	end)
...
local function CreatePartialEffects()
	...
	-- Close button.
	local button = Theme:CloseButton(frame, function() frame:Hide() end)
...
local function CreateDamageColors()
	...
	for damageType, profileKey in pairs(MSBTMain.damageColorProfileEntries) do
		...
		checkbox:Configure(16, MSBTMain.damageTypeMap[damageType], objLocale.tooltip)
		checkbox:SetPoint("LEFT", colorswatch, "RIGHT", 5, 0)
		checkbox:SetClickHandler(function(this, isChecked)
...
local function CreateClassColors()
	...
	for class in string.gmatch(classString, "[^%s]+") do
		...
		checkbox:Configure(16, CLASS_NAMES[class], objLocale.tooltip)
		checkbox:SetPoint("LEFT", colorswatch, "RIGHT", 5, 0)
		checkbox:SetClickHandler(function(this, isChecked)
...
local function CreateClasses()
	...
	for class in string.gmatch(classString, "[^ ]+") do
		checkbox = MSBTControls.CreateCheckbox(frame)
		checkbox:Configure(16, CLASS_NAMES[class], nil)
...
module.ShowProfileExport			= ShowProfileExport
module.ShowProfileImport			= ShowProfileImport
